...
A = (0,0);C = (0,1);G = (1,0);T = (1,1) --JC(level2)
...
L2 = {a_0,a_1,b_0,b_1,c_0,c_1,d_0,d_1,e_0,e_1,f_0,f_1,g_0,g_1,h_0,h_1,j_0,j_1};--JC(level2)
...
R = QQ[(L2|L1), MonomialOrder=>Eliminate 18] --JC(level2)
...
-a_(x#(i#0))*b_(x#(i#0))*c_0*f_(x#(i#0))*g_(x#(i#2))*h_(x#(i#2))*j#(x#(i#1))
-a_(x#(i#0))*b_(x#(i#1))*c_(x#(i#2))*e_(x#(i#2))*f_(x#(i#1))*h_(x#(i#2))*j_(x#(i#1))
-a_(x#(i#0))*c_(x#(i#0))*d_(x#(i#0))*f_(x#(i#0))*g_(x#(i#2))*h_(x#(i#2))*j_(x#(i#1))
-a_(x#(i#0))*c_(x#(i#0))*d_(x#(i#1))*e_(x#(i#2))*f_(x#(i#1))*h_(x#(i#2))*j_(x#(i#1))
);
...
--The parameteriztaion for the 3-leaf, 3-cycle network with no
-- reticulation leaf edge
...
--The parameterization for the 3-leaf, 3-cycle network without
-- one of the non-reticulation 
--leaves
...
restart
needs "3-leaf-level-2.m2"

K = ideal(Eqn_1);
time GG = selectInSubring(1,gens gb(K, DegreeLimit => 16))
I = ideal(GG)
J = eliminate(K,{a_0,a_1,b_0,b_1,c_0,c_1,d_0,d_1,e_0,e_1,f_0,f_1,g_0,g_1,h_0,h_1,j_0,j_1})
dim I
dim K
